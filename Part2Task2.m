...
lambda_vec = linspace(0, 1, 100); % taper ration c_t/c_r
AR_vec = [4, 6, 8, 10]; % aspect ratios from figure 5.20
delta_mat  = zeros(length(AR_vec), length(lambda_vec));
...
for i = 1:length(AR_vec)
    ...
    for j = 1:length(lambda_vec)
        lambda = lambda_vec(j);
        c_t = lambda * c_r;  % tip chord from taper ratio
        c_avg = (c_t + c_r) / 2; % average chord length to find S

        b = AR * c_avg; % AR = b/c_avg
    
        %Call funtion
        [e,c_L,c_Di] = PLLT(b,a0_t,a0_r,c_t,c_r,aero_t,aero_r,geo_t,geo_r,N);
    
        % induced drag factor delta from e (delta = (1/e) - 1
        delta_mat(i, j) = 1./e - 1; % delta matrix
    end
end
...
hold on
for i = 1:length(AR_vec)
    plot(lambda_vec, delta_mat(i,:), 'LineWidth', 1.5);
end
xlabel('Taper Ratio, c_t / c_r');
ylabel('\delta');
title('Induced Drag Factor \delta vs. Taper Ratio c_t / c_r');
